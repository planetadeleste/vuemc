...
class Collection extends Base {
  private readonly _models!: Ref<Model[]>;
  ...
  private readonly _registry!: Ref<Record<string, string>>;
  ...
  /** @returns {Model} The class/constructor for this collection's model type. */
  model(): typeof Model {
  ...
  /**
   * Syncs all models in this collection. This method delegates to each model so
   * follows the same signature and effects as `Model.sync`.
   */
  sync(): void {
  ...
  validate(): Promise<ValidationResultErrorFinalResult[]> {
    let validations = this.models.map(
      (model): Promise<ValidationResultErrorFinalResult> => model.validate()
    );

    return Promise.all(validations).then(
      (
        errors: ValidationResultErrorFinalResult[]
  ...
  addModelToRegistry(model: Model): void {
    set(this._registry.value, model._uid, 1);
  }
   ...
   *   object, or an array of either, to this collection. A model instance will
   *   be created and returned if passed a plain object.
  ...
  add(
    model?: Model | Model[] | Partial<Model> | Record<string, any>
  ): Model | Model[] | Partial<Model> | Record<string, any> | void {
    ...
    if (isPlainObject(model)) {
      return this.add(
        this.createModel(model as Partial<Model> | Record<string, any>)
      );
    }
   ...
   *   `Model` instance, an object to filter by, a function to filter by, or an
   *   array of any of the above to remove multiple.
  ...
  remove(
    model: Model | Model[] | Partial<Model> | ((model: Model) => boolean)
  ...
  indexOf(model: Model): number {
    ...
    if (this.isModel(filter)) {
      ...
      if (!has(this._registry.value, model._uid)) {
        return -1;
      }
   ...
   * running each model through `iteratee`, where each successive invocation is
   * supplied the return value of the previous.
   *
   * If `initial` is not given, the first model of the collection is used as the
   * initial value.
   *
   ...
   *   which values to sum by. Invoked with a single argument `model`.
   ...
   * @returns {Object}
   */
  getPaginationQuery(): { page: number | null | undefined } {
    return {
      page: this._page.value,
    };
  }
  ...
  applyValidationErrorObject(
    ...
    let lookup: Record<string, Model> = keyBy(this.models, (model): string =>
      model.identifier()
    );
  ...
  onSave(): Promise<RequestOperation> {
    ...
    let valid = true;
    let tasks: Promise<RequestOperation | void>[] = this.models.map(
      (model): Promise<RequestOperation | void> => {
  ...
  onDelete(): Promise<RequestOperation> {
    ...
    return Promise.all(
      this.models.map((m): Promise<RequestOperation> => m.onDelete())
