...
const copyFrom = function (
  ...
  each(source, (value, key): void => {
    if (isArray(value)) {
      ...
      isObject(value) &&
      isFunction((value as Collection | Model).clone)
    ) {
...
class Model<A extends Record<string, any> = Record<string, any>> extends Base {
  ...
  private readonly _attributes!: Ref<A | Record<string, any>>;
  private readonly _collections!: Ref<Record<string, Collection>>;

  private readonly _reference!: Ref<Record<string, any>>;
  private _mutations!: Ref<Record<string, Mutation>>;
  private readonly _errors!: Ref<Record<string, string[]>>;

  /**
   * A convenience wrapper around the model's attributes that are saved. This is
   * similar to the `saved` method, but instead of accessing a single property
   * it returns the whole saved object, so that you can do something like
   * model.$.attribute when you want to display it somewhere.
   *
  ...
  get collections(): Collection[] {
    return values(this._collections.value);
  }
  ...
  clone(): Model<A> {
    ...
    copyFrom(this._attributes.value, attributes);
    ...
    clone._reference.value = reference;
    clone._attributes.value = attributes;

    return clone;
  }

  /**
   * Prepare certain methods to only be called once. These are methods that are
   * expected to return the same data every time.
   *
  ...
  compileMutators(): void {
    this._mutations.value = mapValues(
      this.mutations(),
      (m: Mutation | Mutation[]): Mutation => flow(m as Mutation[])
    );
  }
  ...
  registerCollection(collection: Collection | Collection[]): void {
    ...
    set(this._collections.value, collection._uid, collection);
  }
  ...
  unregisterCollection(collection: Collection): void {
    ...
    unset(this._collections.value, collection._uid);
  }
  ...
  clearAttributes(): void {
    ...
    this._attributes.value = cloneDeep(defaults);
    this._reference.value = cloneDeep(defaults);
  }
  ...
  /**
   * Assigns all given model data to the model's attributes and reference. This
   * will also fill any gaps using the model's default attributes.
   *
  ...
  reset(attribute: string | string[]): void {
    ...
    if (attribute) {
      ...
      copyFrom(this._reference.value, this._attributes.value);
    }
  ...
  mutated(attribute: string, value: any): any {
    let mutator: Mutation = get(this._mutations.value, attribute);
  ...
  mutate(attribute?: string | string[]): void {
    if (isUndefined(attribute)) {
      each(this.attributes, (value, attribute): void => {
        set(this._attributes.value, attribute, this.mutated(attribute, value));
      });
      ...
      each(castArray(attribute), (attribute): void => {
        ...
        set(this._attributes.value, attribute, mutated);
      });
    }
  }
  ...
  sync(attribute?: string | string[]): void {
    ...
    let active: Record<string, any> = cloneDeep(this.attributes);

    // Sync either specific attributes or all attributes if none provided.
    if (isUndefined(attribute)) {
      this._reference.value = active;
    } else {
      each(castArray(attribute), (attribute): void => {
        set(this._reference.value, attribute, get(active, attribute));
      });
    }
  ...
  /**
   * Registers an attribute on this model so that it can be accessed directly on
   * the model, passing through `get` and `set`.
   */
   ...
   * that is compatible with Vue's reactivity. This method should always be used
  ...
  set<T = any>(
    ...
    set(this._attributes.value, attribute as string, value);
  ...
  /**
   * Reverts all attributes back to their defaults, or `undefined` if a default
  ...
  unset(attribute: string | string[]): void {
    ...
    let attributes: string | string[] = defaultTo(
      attribute,
    ...
    each(castArray(attributes), (attribute): void => {
      if (this.has(attribute)) {
        set(this._attributes.value, attribute, get(defaults, attribute));
      }
    });
  }
  ...
  validateAttribute(
    ...
    let tasks: ValidationTask[] = rules.map((rule): true | string =>
      rule(value, attribute, this)
    );
    ...
    return (Promise.all(tasks) as Promise<ValidationResultError[]>).then(
      (errors: ValidationResultError[]): ValidationResultErrorFinalResult => {
        ...
        errors = filter(
          errors,
          (e): boolean => isString(e) || isObject(e)
        ) as ValidationResultError[];
  ...
  validate(
    ...
    if (isString(attributes)) {
      return this.validateAttribute(attributes).then(
        (errors): Record<string, string> => {
          return (
            !isEmpty(errors) ? { [attributes as string]: errors } : {}
    ...
    if (isArray(attributes)) {
      ...
      let tasks: Promise<void>[] = attributes.map(
        (attribute): Promise<void> => {
          return this.validateAttribute(attribute).then(
            (errors: ValidationResultErrorFinalResult): void => {
              if (!isEmpty(errors)) {
      ...
      return Promise.all(tasks).then(
        (): Record<string, ValidationResultErrorFinalResult> => $errors
      );
    }
  ...
  addToAllCollections(): void {
    each(this._collections.value, (collection): void => {
      collection.add(this);
    });
  }
  ...
  getSaveData(): A | Record<string, any> {
  ...
  /**
   * Sets errors for a specific attribute. Support the ability to clear error by
  ...
  setAttributeErrors(
    ...
    if (isEmpty(errors)) {
      ...
      set(this._errors.value, attribute, castArray(errors));
    }
  }
  ...
  getErrors(): Record<string, string | string[]> {
    if (this.getOption('useFirstErrorOnly')) {
      return mapValues(this._errors.value, head) as Record<string, string>;
    }
  ...
  onSaveValidationFailure(error: ResponseError): void {
    let errors: Record<string, any> | null = error
      .getResponse()!
      .getValidationErrors();
  ...
  /**
   * Called when a save request resulted in an unexpected error, eg. an internal
   * server error (500)
   *
   * @param {Error} error
   * @param {Object} response
   ...
   *   been created yet. The default test is to check if the model's identifier
   *   is missing.
   */
  isNew(): boolean {
  ...
  /**
   * @returns {boolean} The opposite of `isNew`, returns `true` if this model is
   *   already persisted somewhere else.
   */
  isExisting(): boolean {
    return !this.isNew();
  }
...
interface ModelOptions extends Options {
  ...
  /**
   * Whether this model should perform a "patch" on update, which will only send
   * changed attributes in the request.
   */
  patch?: boolean;

  /**
   * Whether this model should save even if no attributes have changed since the
   * last time they were synced. If set to `false` and no changes have been
   * made, the request will be considered a success.
   */
  saveUnchanged?: boolean;

  /**
   * Whether this model should only use the first validation error it receives,
   * rather than an array of errors.
   */
  ...
  /**
   * Whether this model should be validated before it is saved. This will cause
   * the request to fail if validation does not pass. This is useful when you
   * only want to validate on demand.
   */
  validateOnSave?: boolean;

  /**
   * Whether this model should validate models and collections within its
   * attribute tree. The result is implicit recursion as each of those instances
   * will also validate their trees, etc.
   */
  validateRecursively?: boolean;

  /**
   * Whether this model should mutate a property as it is changed, before it is
   * set. This is a rare requirement because you usually don't want to mutate
   * something that you are busy editing.
   */
  mutateOnChange?: boolean;

  /**
   * Whether this model should mutate all attributes before they are synced to
   * the "saved" state. This would include construction, on fetch, on save, and
   * on assign.
   */
